...
static const u32 coop_threshold = 500;
static const u64 quantization_base = 1000;
static const u64 default_delta_param = 40;
static const u64 default_max_param = 500;
			       ...
			       u64 now, u32 acked, u32 lost)
{
	...
	if (copa->mode == Default) {
		...
		if (copa->delta_param < default_delta_param) {
			copa->delta_param = default_delta_param;
		}
...
static void copa_main(struct sock *sk, const struct rate_sample *rs)
{
	...
	if (increase_cwnd) {
		if (!copa->is_slow_start) {
			...
			u32 new_cwnd = cwnd_bytes + copa->report_acked_bytes;
			tp->snd_cwnd = min(new_cwnd / tp->mss_cache,
					   tp->snd_cwnd_clamp);
		}
	} else {
...
static void copa_init(struct sock *sk)
{
	...
	if (mortise->copa != NULL) {
		...
		copa->prior_cwnd = 10;
		copa->last_report_time = tcp_clock_us();
...
MODULE_DESCRIPTION("Copa MVFST in Kernel Module");
